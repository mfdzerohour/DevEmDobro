...
// export const HomeView = () => {
//     const [pokemons, setPokemons] = useState([]); // Lista de Pokémons exibidos
//     const [allPokemons, setAllPokemons] = useState([]); // Lista completa de Pokémons
//     const [loading, setLoading] = useState(true); // Estado de carregamento
//     const [offset, setOffset] = useState(0); // Controle de deslocamento para carregamento incremental
...
//                 const initialPokemonData = res.map((response) => ({
//                     name: response.data.name,
//                     image: response.data.sprites.other["official-artwork"].front_default, // URL da imagem oficial
//                     types: response.data.types, // Tipos do Pokémon
//                     moves: response.data.moves, // Movimentos do Pokémon
//                     abilities: response.data.abilities, // Habilidades do Pokémon
//                 }));
...
//             })
//             .catch((err) => {
//                 console.error("Erro ao carregar Pokémons iniciais:", err);
//                 setLoading(false); // Desativa o estado de carregamento em caso de erro
//             });
//     };
...
//     const pokemonFilter = (name) => {
//         if (name === "") {
//             setPokemons(allPokemons); // Restaura todos os Pokémons se a pesquisa estiver vazia
//         } else {
//             const filteredPokemons = allPokemons.filter((pokemon) =>
//                 pokemon.name.toLowerCase().includes(name.toLowerCase())
//             );
